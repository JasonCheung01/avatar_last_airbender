...
```{r}
dat |> filter(is.na(character_words)) |> select(character) |> unique()
```

For the `imdb_rating` column, all missing values correspond to a single episode.
```{r}
dat |> filter(is.na(imdb_rating)) |> group_by(book,chapter) |> summarise(n(),.groups ="keep")
```
