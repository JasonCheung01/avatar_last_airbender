...
full_avatar |>
  ...
  select(full_text)
```
Lets use this to identify a change of scene, which will be usefull for character interactions
```{r}
full_avatar <- full_avatar |> 
  mutate(new_scene=case_when(is.na(character_words) & is.na(scene_description) ~ 1,
                                          TRUE ~ 0))
full_avatar |> head()
```
Adding an scene_id
```{r}
full_avatar <- full_avatar |> mutate(scene_id=cumsum(new_scene))
```
...
write.csv(full_avatar, "avatar_data_scene.csv", row.names = FALSE)
```
